...
function verificarCadastroCompleto(user) {
  ...
  db.collection('clientes').doc(user.uid).get()
    .then((doc) => {
      ...
      if (!doc.exists) {
        console.log("Documento do usuário não existe. Redirecionando para cadastro...");
  ...
  if (typeof firebase !== 'undefined' && firebase.auth) {
    ...
    firebase.auth().onAuthStateChanged(function(user) {
      if (user) {
        ...
        if (!window.location.href.includes('let-you-screen.html') && 
    ...
    console.error("Firebase Auth não está disponível. Verifique se o SDK do Firebase foi carregado corretamente.");
  }
});
