...
$(document).on("click", ".eye-off", function () {
  ...
  $(this).attr(
    "src",
    isPassword ? "assets/images/svg/eye.svg" : "assets/images/svg/eye-off.svg"
  );
});
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  function toggleSection(targetSection) {
    ...
    const targetButton = document.querySelector(
      `.toggle-btn-per-info[data-section="${targetSection}"]`
    );
...
function continueAction() {
  ...
  if (!selectedAction) {
    showInfoPopup("Por favor, selecione uma opção antes de continuar.");
    return;
  }
...
document.querySelectorAll(".check-radio2").forEach((radio) => {
  radio.addEventListener("change", function () {
    document
      .querySelectorAll(".news-tex")
      .forEach((text) => (text.style.color = "#0F0F0F"));
    this.closest(".your-data-main").querySelector(".news-tex").style.color =
      "#6218FF";
  });
});
...
document.addEventListener("DOMContentLoaded", function () {
  ...
  if (pitchButton && speedButton && pitchPopup && speedPopup) {
    function showPopup(popup, button) {
      ...
      document
        .querySelectorAll(".display-value")
