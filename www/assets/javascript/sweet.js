...
!(function (e, t) {
  ...
  function de(e) {
    ...
    e.style.setProperty(
      ...
      t.getPropertyValue("--swal2-outline").replace(
        / rgba\(.*/,
        ` $ {
                        n
                    }
  ...
  const ge = [
    ...
    fe = (e) => {
      if (!e.input) return;
      if (!Ee[e.input])
        return void d(`Unexpected type of input ! Expected $ {
                    Object.keys(Ee).join(" | ")
                }
  ...
  const Bt = (e) => {
    ...
    Pt = (e, t) => {
      const n = he.innerParams.get(e);
      if (!n.input)
        return void d(`The "input"parameter is needed to be set when using returnInputValueOn$ {
                    c(t)
                }
  ...
  const kn = ["swal-title", "swal-html", "swal-footer"],
    ...
    Mn = (e, t) => {
      Array.from(e.attributes).forEach((n) => {
        -1 === t.indexOf(n.name) &&
          u([
            `Unrecognized attribute "${n.name}"on <$ {
