  ...
  if (!idSolicitacao) {
    ...
    firebase.auth().onAuthStateChanged(async function (user) {
      if (user) {
        ...
        const docRef = db.collection("investimento").doc(idSolicitacao);
      ...
      try {
        if (idSolicitacao) {
          ...
          Swal.fire({
            ...
            window.location.href = "investimento-list.html";
          });
        } else {
          // Cria uma nova solicitação
          await firebase.firestore().collection("investimento").add(dados);
