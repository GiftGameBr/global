...
const db = firebase.firestore(); // 🔥 adiciona isso
window.db = db; // 🔥 torna acessível globalmente
...
function logoutUser() {
  auth
    ...
    .then(() => {
      ...
      window.location.href = "sign-in.html"; // Ajuste o nome se necessário
    })
    .catch((error) => {
...
function loginUser() {
  ...
  if (!email || !password) {
    showErrorPopup("Por favor, preencha o email e a senha.");
    return;
  }
  ...
  auth
    ...
    .then(() => {
      redirecionarPosLogin(); // Função que redireciona após o login
    })
    .catch((error) => {
      showErrorPopup("Erro ao realizar login: " + error.message);
    });
}
...
function updateWelcomeMessage(user) {
  ...
  if (welcomeElement && user.email) {
    const userEmail = user.email.split("@")[0]; // Pega parte antes do @ do email
    welcomeElement.textContent = `Bem Vindo, ${userEmail}!`;
  }
}
